...
CONFIG="${CONFIG:-configs/mid_laya_recipe_lr5x.yaml}"
WORK="${WORK:-/kaggle/working}"
DATA_DIR="${DATA_DIR:-$WORK/data_mid_laya_lr5x}"
OUT_DIR="${OUT_DIR:-$WORK/laya_medical_mid_laya_lr5x}"
...
echo "=== torchrun train_ddp (Laya recipe mid, LR×5, 5 epochs) ==="
...
python scripts/evaluate.py \
  ...
  --eval-packs "$DATA_DIR/eval_packs.pt" \
  --out "$WORK/eval_report_mid_laya_lr5x.json"

python - <<PY
import json
from pathlib import Path
report = json.load(open("$WORK/eval_report_mid_laya_lr5x.json"))
...
summary = {
  "ok": True,
  "recipe": "laya_ft_ce_soft_teacher_mid50k_lr5x_e5",
  "nproc": int("$NPROC"),
...
Path("$WORK/summary_mid_laya_lr5x.json").write_text(json.dumps(summary, indent=2))
