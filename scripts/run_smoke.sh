#!/usr/bin/env bash
# Smoke medical fine-tune: prepare data -> torchrun DDP train -> dual eval.
set -euo pipefail
...
OUT_DIR="${OUT_DIR:-$WORK/laya_medical_smoke}"
...
echo "=== prepare_data (force rebuild; do not reuse stale train_items.pt) ==="
...
python scripts/evaluate.py \
  ...
  --out "$WORK/eval_report.json"

python - <<PY
import json
from pathlib import Path
report = json.load(open("$WORK/eval_report.json"))
summary = {
  "ok": True,
  "nproc": int("$NPROC"),
  "output_dir": "$OUT_DIR",
...
Path("$WORK/summary.json").write_text(json.dumps(summary, indent=2))
