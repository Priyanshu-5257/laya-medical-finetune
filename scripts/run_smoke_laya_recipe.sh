#!/usr/bin/env bash
# Smoke: Laya FT recipe (CE + soft teacher + sph=0.75) on medical smoke data.
set -euo pipefail
...
CONFIG="${CONFIG:-configs/smoke_laya_recipe.yaml}"
WORK="${WORK:-/kaggle/working}"
DATA_DIR="${DATA_DIR:-$WORK/data_laya_recipe}"
OUT_DIR="${OUT_DIR:-$WORK/laya_medical_smoke_laya_recipe}"
...
echo "=== prepare_data (force rebuild; soft teacher targets) ==="
...
echo "=== torchrun train_ddp (Laya recipe: CE + soft targets) ==="
...
python scripts/evaluate.py \
  ...
  --eval-packs "$DATA_DIR/eval_packs.pt" \
  --out "$WORK/eval_report_laya_recipe.json"

python - <<PY
import json
from pathlib import Path
report = json.load(open("$WORK/eval_report_laya_recipe.json"))
meta = json.load(open("$DATA_DIR/data_meta.json"))
summary = {
  "ok": True,
  "recipe": "laya_ft_ce_soft_teacher",
  "nproc": int("$NPROC"),
  "output_dir": "$OUT_DIR",
  "ce_weight": 1.0,
  "sph_weight": 0.75,
  ...
  "vs_prior_pure_rlcd_smoke": {
    "medical_delta": 0.01375,
    "generic_delta": -0.005,
    "note": "prior smoke used buggy MedMCQA cop map; this run uses fixed 0-based labels",
  },
}
Path("$WORK/summary_laya_recipe.json").write_text(json.dumps(summary, indent=2))
print(json.dumps(summary, indent=2))
PY

echo "DONE_LAYA_RECIPE"
