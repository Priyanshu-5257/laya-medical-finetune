"""DDP RLCD fine-tune with PEFT LoRA on middle encoder layers only."""
...
def parse_args():
    ...
    ap.add_argument("--config", default="configs/smoke_peft_middle.yaml")
...
def _save_merged_checkpoint(model, tok, cfg, output_dir, extra_meta=None):
    os.makedirs(output_dir, exist_ok=True)
    # Merge LoRA into base encoder so evaluate.py / laya.load work unchanged.
    model.encoder = merge_encoder_if_peft(model.encoder)
    sd = {k: v.half().contiguous().cpu() for k, v in model.state_dict().items()}
    # Drop any leftover peft keys if merge failed partially
    sd = {k: v for k, v in sd.items() if "lora_" not in k and "base_model" not in k}
    save_file(sd, os.path.join(output_dir, "model.safetensors"))
    # encoder config lives on the merged module
    enc = model.encoder
    if hasattr(enc, "config"):
        enc.config.save_pretrained(os.path.join(output_dir, "encoder"))
    tok.save_pretrained(os.path.join(output_dir, "tokenizer"))
    with open(os.path.join(output_dir, "rl_agent_config.json"), "w") as f:
        json.dump(cfg, f, indent=2)
    if extra_meta is not None:
        with open(os.path.join(output_dir, "peft_meta.json"), "w") as f:
...
def main():
    ...
    try:
        ...
        if rank == 0:
            print("gradient_checkpointing_enable skipped:", e, flush=True)
    ...
    if not enc_params and not head_params:
        raise RuntimeError("No trainable parameters after PEFT setup")
    param_groups = []
    if enc_params:
    ...
    if rank == 0:
        print(
            f"PEFT-middle DDP world={world_size} | train={len(train_items)} "
            f"(calib={len(calib_items)}) | per_rank={len(my_items)} | "
            f"epochs={EPOCHS} | G={GROUP_SIZE} | ce_weight={CE_WEIGHT}",
            flush=True,
        )
        print("peft_meta:", json.dumps(peft_meta, indent=2), flush=True)
        print(
            f"trainable params: {counts['trainable']:,} / {counts['total']:,} "
            f"({100.0 * counts['trainable'] / max(1, counts['total']):.2f}%)",
            flush=True,
        )
    ...
    for epoch in range(EPOCHS):
        ...
        for b_idx in range(0, len(my_items), MICRO_BATCH):
            ...
            with torch.no_grad():
                ...
                # Per-question group-mean baseline + group std (not batch-wide).
        ...
        if rank == 0:
            ...
            # Rolling adapter-aware dump (not merged); useful if the job dies mid-run.
            ckpt_dir = os.path.join(args.output_dir, "checkpoint_latest")
            os.makedirs(ckpt_dir, exist_ok=True)
            torch.save(model.state_dict(), os.path.join(ckpt_dir, "pytorch_model_peft.pt"))
    ...
    if rank == 0:
        ...
        with torch.no_grad():
            for c_idx in range(0, len(calib_items), 16):
                ...
                for i, it in enumerate(c_chunk):
                    kk = len(it["markers"])
                    calib_preds.append((it["qtype"], l_np[i, :kk], it["target"]))
        ...
        _save_merged_checkpoint(
            ...
            cfg,
            args.output_dir,
            extra_meta={"peft": peft_meta, "trainable": counts, "history": history},
        )
        ...
        print(f"Saved merged checkpoint to {args.output_dir}", flush=True)
