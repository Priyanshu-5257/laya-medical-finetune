#!/usr/bin/env bash
# Mid-scale Laya recipe FT: ~50k Q × 3 epochs.
set -euo pipefail
...
CONFIG="${CONFIG:-configs/mid_laya_recipe.yaml}"
WORK="${WORK:-/kaggle/working}"
DATA_DIR="${DATA_DIR:-$WORK/data_mid_laya}"
OUT_DIR="${OUT_DIR:-$WORK/laya_medical_mid_laya_recipe}"
...
echo "=== torchrun train_ddp (Laya recipe mid-scale) ==="
...
python scripts/evaluate.py \
  ...
  --eval-packs "$DATA_DIR/eval_packs.pt" \
  --out "$WORK/eval_report_mid_laya.json"

python - <<PY
import json
from pathlib import Path
report = json.load(open("$WORK/eval_report_mid_laya.json"))
...
summary = {
  "ok": True,
  "recipe": "laya_ft_ce_soft_teacher_mid50k",
  "nproc": int("$NPROC"),
  "n_train": meta.get("n_train"),
  "soft_targets": meta.get("soft_targets"),
  "epochs": hist.get("epochs", 3),
  ...
  "vs_full_pure_rlcd": {"medical_delta": 0.0275, "best_epoch_medical_delta": 0.05625},
  "vs_smoke_laya_recipe": {"medical_delta": 0.00625, "generic_delta": -0.00375},
}
Path("$WORK/summary_mid_laya.json").write_text(json.dumps(summary, indent=2))
